...
from empkins_io.datasets.d03.macro_ap01 import MacroStudyTsstDatasetPerPhase
...
dataset = MacroStudyTsstDatasetPerPhase(base_path)
...
def process_subset(
    subset: MacroStudyTsstDatasetPerPhase, expert_feature_params, generic_feature_params
) -> Tuple[Tuple[str, str], pd.DataFrame]:
    subject_id = subset.index["subject"][0]
    condition = subset.index["condition"][0]

    phase_dict = {}
    for subsubset in subset.groupby(["phase"]):
        phase = subsubset.index["phase"][0]
        mocap_data = subsubset.mocap_data
        ...
        phase_dict[phase] = pd.concat([generic_features, expert_features])

    return_data = pd.concat(phase_dict, names=["phase"])
...
dataset = dataset.get_subset(phase=["talk", "math"]).groupby(group_levels)
